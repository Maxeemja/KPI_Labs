...
__declspec(align(16)) float arrayA[4800];
__declspec(align(16)) float arrayB[4800];
...
void fill_arrays() {
	...
	{
		arrayA[i] = 1.0 + (15000 - i);
		arrayB[i] = (i % 2) ? -2.0 : 1.0;
	}
}

float MyDotProduct(float* arrayA, float* arrayB, long N) {
	float res_tmp = 0;
	for (long i = 0; i < N; i++) {
		res_tmp += arrayA[i] * arrayB[i];
	}
	return res_tmp;
}
...
void myWorkC(HWND hWnd)
{
	...
	{
		RES = MyDotProduct(arrayA, arrayB, 4800);
	}
	GetLocalTime(&st);
	ten = 60000 * (long)st.wMinute
		+ 1000 * (long)st.wSecond
		+ (long)st.wMilliseconds - tst;
	sprintf_s(TextBuf, "RES = %f\nTIME = %ld ms", RES, ten);
	MessageBoxA(hWnd, TextBuf, "Результат C++", MB_OK);
}
void myWorkFPU(HWND hWnd)
{
	fill_arrays();
	...
	for (long i = 0; i < 1000000; i++)
	{
		MyDotProduct_FPU(&RES, arrayA, arrayB, 4800);
	}
	GetLocalTime(&st);
	ten = 60000 * (long)st.wMinute
		+ 1000 * (long)st.wSecond
		+ (long)st.wMilliseconds - tst;
	sprintf_s(TextBuf, "RES = %f\nTIME = %ld ms", RES, ten);
	MessageBoxA(hWnd, TextBuf, "Результат FPU", MB_OK);
}
void myWorkSSE(HWND hWnd)
{
	fill_arrays();
	...
	for (long i = 0; i < 1000000; i++)
	{
		MyDotProduct_SSE(&RES, arrayA, arrayB, 4800);
	}
	GetLocalTime(&st);
	ten = 60000 * (long)st.wMinute
		+ 1000 * (long)st.wSecond
		+ (long)st.wMilliseconds - tst;
	sprintf_s(TextBuf, "RES = %f\nTIME = %ld ms", RES, ten);
	MessageBoxA(hWnd, TextBuf, "Результат SSE", MB_OK);
}
